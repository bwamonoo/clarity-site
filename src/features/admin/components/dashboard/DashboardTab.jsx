...
export default function DashboardTab() {
  ...
  const { navigateToTab } = useAdmin() // Use the new navigation method
      ...
      case 'confirm-all-pending':
        try {
          ...
          if (pendingAppointments.length === 0) {
            addToast('No pending appointments to confirm', 'info', 3000)
            return
          }
          ...
          if (result.success) {
            addToast(`Successfully confirmed ${pendingAppointments.length} pending appointment(s)`, 'success', 4000)
            refreshDashboard()
          } else {
            addToast('Failed to confirm appointments', 'error', 4000)
          }
        } catch (error) {
          console.error('Error confirming appointments:', error)
          addToast('Error confirming appointments', 'error', 4000)
        }
        break
        
      case 'reply-unread':
        // Navigate to Contacts tab with unread filter
        navigateToTab('contacts', { initialFilter: 'unread' })
        addToast('Showing unread messages', 'info', 3000)
        break
        
      case 'add-service':
        // Navigate to Services tab and open form
        navigateToTab('services', { showForm: true })
        addToast('Ready to add new service', 'info', 3000)
        break
